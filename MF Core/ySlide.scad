...
echo(diff);
	...
	translate([0,(yslideWidth+15)/2-5,0]) cylinder(d=3, h=yslideWidth+5, center=true);	
	...
	difference() {
	translate([-(xRodSpread/2)-7,-7.5-yslideWidth/2,0]) cube([6,12,yslideWidth], center=true);
	translate([-(xRodSpread/2)-7,-7.5-yslideWidth/2-3,0]) rotate([0,90,0]) cylinder(d=3, h=120, center=true);
	}
}
