...
module zMountPiece(front=false, width) {
	...
	difference() {
		union(){
			rotate([27.5,0,0]) cube([armwidth, zRodSpread-2*wall+5.2, wall*3], center=true);
			rotate([-27.5,0,0]) cube([armwidth, zRodSpread-2*wall+5.2, wall*3], center=true);
		}
...
module plateTie() {
	difference() {
		union() {
		#	translate([0,-(zRodSpread/2 + lmu8width+wall-.1),0]) cube([(extrusion+wall+.2)*1.5, extrusion+wall, extrusion+wall], center=true);
			translate([0,(zRodSpread/2 + lmu8width+wall-.1),0]) cube([(extrusion+wall+.2)*1.5, extrusion+wall, extrusion+wall], center=true);
			translate([0,0,-wall]) rotate([0,90,0]) union(){
				rotate([11.5,0,0]) cube([extrusion-wall, zRodSpread*1.31, wall*2], center=true);
				rotate([-11.5,0,0]) cube([extrusion-wall, zRodSpread*1.31, wall*2], center=true);
			}
			translate([(extrusion-4.5)/2,0,extrusion+wall])	{
				difference() {
					union(){
						hull() {
							translate([0,(zRodSpread/2 + lmu8width+wall-.1),0]) cube([extrusion+wall, extrusion+wall, extrusion+wall], center=true);
							translate([-(extrusion+wall+.2)/2,(zRodSpread/2 + lmu8width+wall-.1),-(extrusion+wall)/2]) cube([extrusion+wall, extrusion+wall, .1], center=true);
						}
						hull() {
							translate([0,-(zRodSpread/2 + lmu8width+wall-.1),0]) cube([extrusion+wall, extrusion+wall, extrusion+wall], center=true);
							translate([-(extrusion+wall+.2)/2,-(zRodSpread/2 + lmu8width+wall-.1),-(extrusion+wall)/2]) cube([extrusion+wall, extrusion+wall, .1], center=true);
				...
				#	cube([extrusion+.2,200, extrusion+.2], center=true);
					#translate([0,-(zRodSpread/2 + lmu8width+wall-.1), 0]) cylinder(d=3, h=20.1, center=true);
					#translate([0,(zRodSpread/2 + lmu8width+wall-.1), 0]) cylinder(d=3, h=20.1, center=true);
	...
	#	translate([0,-(zRodSpread/2 + lmu8width+wall-.1),0]) cube([200, extrusion+.2, extrusion+.2], center=true);
...
difference() {
	union() {
	cube([(extrusion+wall+.2)*1.5, extrusion+wall, extrusion+wall], center=true);
	translate([0,0,extrusion+wall]) cube([10, extrusion+wall, extrusion+wall], center=true);
}
cube([200, extrusion+.3, extrusion+.3], center=true);
